...
function ensureUserDir() {
  ...
  if (!fs.existsSync(SITES_FILE)) fs.writeFileSync(SITES_FILE, DEFAULT_SITES.join('\n') + '\n');
}
...
function readSites() {
  ...
  } catch { return DEFAULT_SITES.slice(); }
}

function writeSites(list) {
  ensureUserDir();
  fs.writeFileSync(SITES_FILE, list.map(s => s.trim()).filter(Boolean).join('\n') + '\n');
}
...
module.exports = {
  USER_DIR, isInstalled, needsUpdate, installDaemon, uninstallDaemon,
